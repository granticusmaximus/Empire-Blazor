...
namespace Empire.Service
{
    ...
    {
        private readonly ApplicationDbContext conn;

        public ServiceImplementation(ApplicationDbContext conn)
        {
            this.conn = conn;
        }
        ...
        public async Task<(bool flag, string message)> AddNoteAsync(ExpenseNote note)
        {
            conn.ExpenseNotes.Add(note);
            await conn.SaveChangesAsync();
            return (true, "Saved");
        }
        ...
        public async Task<List<ExpenseNote>> GetNoteAsync() => await conn.ExpenseNotes.ToListAsync();
    }
}